...
# -------------------------
class WorkerZMQ:
    def __init__(self, server_addr: str = "tcp://127.0.0.1:5555", worker_id: Optional[str] = None):
    ...
    async def run(self):
        ...
        try:
            # 1) keep asking for available jobs until one is present
            job_id = None
            while self.running and job_id is None:
            ...
            if job_id is None:
                logger.info("No job assigned; exiting worker run")
                return

            # 2) register to that job to get hyperparams, model_json, initial_weights
            reg = await self.register(job_id)
            if reg.get("status") != "ok":
                logger.error("Registration failed: %s", reg)
                return
...
def start_worker():
    """Starts the worker and keeps polling for jobs without unnecessary restarts."""
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument("--ps", default="tcp://127.0.0.1:5555", help="Parameter server address")
    parser.add_argument("--id", default=None, help="Worker id")
    args = parser.parse_args()

    while True:
        try:
            worker = WorkerZMQ(server_addr=args.ps, worker_id=args.id)
            ...
            # Exit cleanly if worker finishes gracefully
            logger.info("No jobs available. Worker will keep polling...")
            import time
